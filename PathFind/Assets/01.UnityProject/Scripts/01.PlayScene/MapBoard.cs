...
public class MapBoard : MonoBehaviour
{
    ...
    //! 맵의 좌표를 받아서 해당 열의 타일을 리스트로 가져오는 함수
    public List<TerrainController> GetTettains_Colum(int xIdx2D)
    {
        return GetTerrains_Colum(xIdx2D, false);
    }   //  GetTettains_Colum()

    //! 맵의 x 좌표를 받아서 행당 열의 타일을 리스로 가져오는 함수
    ...
    public Vector2Int GetDistance2D(GameObject targetTerrainnObj, GameObject destTerrainObj)
    {
        Vector2 localDistance = destTerrainObj.transform.localPosition - targetTerrainnObj.transform.localPosition;
